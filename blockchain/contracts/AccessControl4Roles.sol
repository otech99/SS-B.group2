...
//MANCA LA LOGICA PER LE FUNZIONI RISERVATE A STUDENTE
//N.B. La transazione relativa alle evidenze deve avere successo solo dopo che EnteCert ha certificato lo studente
    ...
    //function permissions_EnteCert(address _contractbnAddress, uint[4] calldata _Evidence, address role) external {
        //if (!hasRole(EnteCert, role)) {
    ...
    // Funzione riservata agli EnteCert
    //function permissions_Azienda(address _contractbnAddress, uint8 _fact_ID, address role) external {
        //if (!hasRole(Azienda, role)) {
            //revert("Caller is not authorized to perform this action");
       // }
       
	    //get_Access_apostInfofacts(_contractbnAddress, _fact_ID); //L'azienda chiama questa funzione per ottenere
   ...
   // }
    ...
    function permissions_Admin(address _contractbnAddress, 
            ...
            Contract_bn.CorsoPyProb calldata _CorsoPyprob,
            Contract_bn.FondInfoProb calldata _FondInfoprob,
            Contract_bn.IngSoftProb calldata _IngSoftprob, address role) external {
