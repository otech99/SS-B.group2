from brownie import accounts, Contract_bn2 
...
def role_management(ruolo_simulato, studente_target_id):
    ...
    contract_bn = Contract_bn2.at(addr)
    ...
    dati_studenti = {
        1: {
            'BasiProg': int(cv_inf['BasiProg'] * Fattore) if cv_s1 == 1 else int(cv_ele['BasiProg'] * Fattore),
            'ProgPy':   int(cv_inf['ProgPy']   * Fattore) if cv_s1 == 1 else int(cv_ele['ProgPy']   * Fattore),
            'evidenze': evidenze_s1,
        },
        2: {
            'BasiProg': int(cv_inf['BasiProg'] * Fattore) if cv_s2 == 1 else int(cv_ele['BasiProg'] * Fattore),
            'ProgPy':   int(cv_inf['ProgPy']   * Fattore) if cv_s2 == 1 else int(cv_ele['ProgPy']   * Fattore),
            'evidenze': evidenze_s2,
        },

        3: {
            'BasiProg': int(cv_inf['BasiProg'] * Fattore) if cv_s3 == 1 else int(cv_ele['BasiProg'] * Fattore),
            'ProgPy':   int(cv_inf['ProgPy']   * Fattore) if cv_s3 == 1 else int(cv_ele['ProgPy']   * Fattore),
...
def main():
    ...
    ruolo = "Azienda"    # "Admin", "Studente", "EnteCert", "Azienda"
    studente = 1    # Inserire il numero dello studente di cui vogliamo fare il test
